                ...
                <div class="p-6">
                    ...
                    <form action="/sales" method="POST" id="sales-form" class="space-y-5">
                        ...
                        <div>
                            ...
                            <div id="sale-items" class="space-y-4"></div>
                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function createSaleRow(initialData = {}) {
                ...
                row.dataset.price = '0';
                row.dataset.stock = '0';
                row.className = 'rounded-2xl border border-slate-200 bg-slate-50/80 p-4 shadow-sm';
                ...
                row.innerHTML = `
                    <div class="mb-4 flex items-center justify-between gap-3">
                        <div>
                            <p class="text-xs font-bold tracking-widest text-slate-500" data-row-label>Barang</p>
                            <p class="mt-1 text-xs text-slate-400">Pilih produk lalu isi jumlah pcs.</p>
                        </div>
                        <button type="button" data-remove-item
                            class="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-500 transition hover:border-red-200 hover:text-red-600">
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-12">
                        <div class="md:col-span-8">
                            <label for="${selectId}" class="mb-1.5 block text-xs font-bold tracking-wider text-slate-500">Produk</label>
                            <select id="${selectId}" name="products[${rowId}][product_id]" class="w-full" required></select>

                            <div data-product-info class="mt-4 hidden space-y-2 rounded-xl border border-slate-100 bg-white p-3">
                        ...
                        <div class="md:col-span-4">
                            <label for="${quantityId}" class="mb-1.5 block text-xs font-bold tracking-wider text-slate-500">Jumlah (Pcs)</label>
                            <input id="${quantityId}" type="number" data-quantity-input name="products[${rowId}][quantity]" min="1" value="${initialData.quantity ?? 1}"
                                class="w-full rounded-xl border-slate-200 bg-white px-4 py-2.5 text-sm transition focus:border-secondary focus:bg-white focus:ring-4 focus:ring-secondary/10"
                                required>

                            <div class="mt-4 rounded-xl bg-slate-100 px-4 py-3 text-xs font-semibold text-slate-500">
                                Subtotal: <span class="font-black text-slate-900">Rp <span data-item-subtotal>0</span></span>
                            </div>
                        </div>
                    </div>
                `;
